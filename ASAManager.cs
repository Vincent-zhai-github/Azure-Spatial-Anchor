...
public class ASAManager : MonoBehaviour
{
    ...
    TaskCompletionSource<bool> located;
    ...
    /// 同步Anchor
    /// </summary>
    /// <param name="anchorId">从服务端获取到的AnchorId</param>
    /// <returns></returns>
    public async Task<bool> SyncAnchor(string[] anchorIds)
    {
        bool started = await startASA();
        if (!started)
        {
        ...
        located = new TaskCompletionSource<bool>();
        ...
        try
        {
            if ((cloudManager != null) && (cloudManager.Session != null))
            {
                ...
                bool success = await located.Task;
                located = null;
    ...
    {
        await SyncAnchor(anchorIDs.ToArray());
    }
